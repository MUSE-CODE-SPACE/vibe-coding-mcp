...
import { CodeAnalysis, ClassInfo, FunctionInfo, ImportInfo } from './astParser.js';
...
export function generateDependencyGraph(
  analyses: { filename: string; analysis: CodeAnalysis }[],
  ...
  for (const { filename, analysis } of analyses) {
    ...
    for (const imp of analysis.imports) {
      if (!imp.source.startsWith('.')) continue; // 외부 패키지 제외
...
export function generateClassDiagram(
  ...
  for (const cls of classes) {
    ...
    for (const prop of cls.properties) {
      if (!showPrivate && prop.visibility === 'private') continue;
    ...
    for (const method of cls.methods) {
      if (!showPrivate && method.name.startsWith('_')) continue;
...
export function generateSequenceDiagram(
  participants: string[],
  interactions: { from: string; to: string; message: string; type?: 'sync' | 'async' | 'return' }[]
): string {
...
export function generateArchitectureDiagram(
  components: { name: string; type: 'service' | 'database' | 'api' | 'client' | 'queue' }[],
...
export function generateGanttChart(
  title: string,
  tasks: { name: string; start: string; duration: string; section?: string }[]
): string {
  const lines: string[] = [
    'gantt',
    `    title ${title}`,
...
export function generateERDiagram(
  entities: { name: string; attributes: { name: string; type: string; pk?: boolean; fk?: boolean }[] }[],
...
export function generateTimeline(
  title: string,
  events: { period: string; events: string[] }[]
): string {
  const lines: string[] = [
    'timeline',
    `    title ${title}`
...
export function generateDiagramsFromAnalysis(
  analysis: CodeAnalysis,
  filename: string
): { type: string; diagram: string }[] {
  const diagrams: { type: string; diagram: string }[] = [];
...
function generateDependencyDiagramFromImports(filename: string, imports: ImportInfo[]): string {
