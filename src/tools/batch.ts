...
const jobHistory: Map<string, BatchJob> = new Map();
const activeJobs: Map<string, { cancelled: boolean }> = new Map();
...
const toolRegistry: Record<string, (params: any) => Promise<any>> = {
  muse_collect_code_context: async (p: any) => collectCodeContext(p),
  muse_summarize_design_decisions: summarizeDesignDecisions,
  muse_generate_dev_document: async (p: any) => generateDevDocument(p),
  muse_normalize_for_platform: async (p: any) => normalizeForPlatform(p),
...
function generateOpId(index: number, tool: string): string {
  return `op_${index}_${tool.replace(/^muse_/, '')}`;
}

// Sort operations by dependencies using topological sort
function sortByDependencies(operations: BatchOperation[]): BatchOperation[] {
  const opMap = new Map<string, BatchOperation>();
