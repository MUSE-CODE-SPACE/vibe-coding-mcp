...
import { listSessions, getStorageStats } from '../core/sessionStorage.js';
...
function formatPeriodLabel(date: Date, period: string): string {
  switch (period) {
    case 'day':
...
// Main tool function
export async function sessionStatsTool(input: SessionStatsInput): Promise<SessionStatsOutput> {
  const {
    action,
    since,
    until,
  ...
  try {
    ...
    if (filterTags && filterTags.length > 0) {
      filteredSessions = filteredSessions.filter((s: any) =>
        s.tags?.some((t: string) => filterTags.includes(t))
      );
    }
    ...
    switch (action) {
      case 'overview': {
        ...
        filteredSessions.forEach((s: any) => {
          if (s.codeContexts) {
            s.codeContexts.forEach((ctx: any) => {
              if (ctx.codeBlocks) {
                totalCodeBlocks += ctx.codeBlocks.length;
                ctx.codeBlocks.forEach((cb: any) => {
                  if (cb.language) languageSet.add(cb.language);
                });
              }
            });
          }
      ...
      case 'timeline': {
        const timeMap = new Map<string, { sessions: number; codeBlocks: number; decisions: number }>();
        ...
        const timeline: TimelineStat[] = Array.from(timeMap.entries())
          .map(([period, stats]) => ({
            period,
      ...
      case 'tags': {
        const tagMap = new Map<string, { count: number; sessions: Set<string> }>();
        ...
        filteredSessions.forEach((s: any) => {
          if (s.tags) {
            s.tags.forEach((tag: string) => {
              ...
              // Track co-occurrence
              s.tags.forEach((otherTag: string) => {
                if (otherTag !== tag) {
      ...
      case 'productivity': {
        ...
        const productivity: ProductivityStat = {
      ...
      case 'trends': {
        ...
        const createTrend = (metric: string, curr: number, prev: number): TrendStat => {
    ...
    logger.error('Session stats failed', error as Error);
    return {
      success: false,
      action,
...
async function getLanguageStats(sessions: any[]): Promise<LanguageStat[]> {
  const langMap = new Map<string, { count: number; lines: number }>();
...
async function getTagStats(sessions: any[]): Promise<TagStat[]> {
