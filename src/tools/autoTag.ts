...
interface TagConfig {
  ...
  customPatterns: Array<{ pattern: string; tags: string[] }>;
}
...
const DEFAULT_PATTERNS: Array<{ patterns: RegExp[]; tags: string[]; category: string }> = [
  // Language patterns
...
function detectTagsFromContent(
  content: string,
  ...
  if (codeBlocks) {
    codeBlocks.forEach((cb) => {
      if (cb.language) fullText += ` ${cb.language}`;
      if (cb.code) fullText += ` ${cb.code}`;
    });
  }
...
Keep tags lowercase, single words or hyphenated.`,
  ...
  try {
    const parsed = JSON.parse(jsonMatch[0]) as TagSuggestion[];
...
// Main tool function
export async function autoTagTool(input: AutoTagInput): Promise<AutoTagOutput> {
  const {
    action,
    sessionId,
    content,
  ...
  try {
      ...
      case 'suggest': {
        ...
        if (sessionId) {
          ...
          if (session.codeContexts) {
            blocks = [];
            session.codeContexts.forEach((ctx: any) => {
              if (ctx.codeBlocks) {
    ...
    logger.error('Auto-tag failed', error as Error);
    return {
      success: false,
      action,
      usedAI: false,
