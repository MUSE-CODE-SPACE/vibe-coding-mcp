...
async function getStatus(repoPath: string, includeUntracked: boolean): Promise<GitStatusResult> {
...
async function getBranches(
  ...
  // Get local branches
  const localFormat =
    '%(refname:short)|%(objectname:short)|%(upstream:short)|%(upstream:track,nobracket)';
  ...
  // Get remote branches
  let remote: Array<{ name: string; lastCommit?: string }> = [];
  if (includeRemote) {
...
async function captureSnapshot(
  ...
  let recentCommits: GitCommit[] | undefined;
  if (includeLog) {
    recentCommits = await getLog(repoPath, { limit: logLimit });
  }

  let currentDiff: GitDiffResult | undefined;
  if (includeDiff && !status.isClean) {
    currentDiff = await getDiff(repoPath, { diffType: 'all', stat: true });
  }

  let stashes: Array<{ index: number; message: string; date: string }> | undefined;
  if (includeStash) {
...
async function extractDecisions(
  ...
  const decisions: GitDesignDecision[] = [];
...
// Main tool function
export async function gitTool(input: GitInput): Promise<GitOutput> {
  ...
  try {
    ...
    logger.error(`Git ${action} failed`, error as Error);
  ...
  inputSchema: {
    ...
    properties: {
      action: {
        ...
        description:
          'Action: status (repo state), log (commit history), diff (changes), branch (branch info), snapshot (full context), extractDecisions (from commits), linkToSession (attach to session)',
      },
      repoPath: {
        type: 'string',
